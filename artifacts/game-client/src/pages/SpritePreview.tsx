...
function SpriteCard({ unit }: { unit: UnitEntry }) {
  const [loaded, setLoaded] = useState(false);
  const [error,  setError ] = useState(false);
  const spriteSrc = `/assets/sprites/${unit.id}.png`;

  return (
    <div className="sp-card" style={{ "--accent": unit.color } as React.CSSProperties}>
      <div className="sp-sprite-area">
        {/* Try to load the real sprite; fall back to placeholder */}
        {!error && (
          <img
            src={spriteSrc}
            alt={unit.name}
  ...
  return (
    <div className="sp-root">
      ...
      <div className="sp-header">
        ...
        <div className="sp-count">{UNITS.filter(u => false).length} / {UNITS.length} sprites loaded</div>
      </div>
      ...
      <div className="sp-grid">
        {visible.map(unit => <SpriteCard key={unit.id} unit={unit} />)}
      </div>
...
const CSS = `
  ...
  .sp-card:hover {
    ...
    box-shadow: 0 6px 24px rgba(0,0,0,0.5), 0 0 18px color-mix(in srgb, var(--accent) 12%, transparent);
